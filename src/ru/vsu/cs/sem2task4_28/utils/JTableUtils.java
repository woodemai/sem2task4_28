...
public class JTableUtils {
    ...
    private static void recalcJTableSize(JTable table) {
        ...
        if (table.getParent() instanceof JViewport && table.getParent().getParent() instanceof JScrollPane scrollPane && (scrollPane.getRowHeader() != null)) {
    ...
    private static void addRowHeader(JTable table, TableModel tableModel, JScrollPane scrollPane) {
        ...
        ListModel<Object> lm = new AbstractListModel<>() {
            @Override
            public int getSize() {
    ...
    public static void initJTableForArray(
            JTable table,
            int defaultColWidth,
            boolean showRowsIndexes,
            boolean showColsIndexes,
            boolean changeRowsCountButtons,
            boolean changeColsCountButtons,
            int changeButtonsSize,
            int changeButtonsMargin
    ) {
    ...
    private static void writeArrayToJTable(JTable table, Object array, String itemFormat) {
        if (!array.getClass().isArray() || !(table.getModel() instanceof DefaultTableModel tableModel)) {
            return;
        }
        ...
        for (int i = 0; i < len1; i++) {
            if (rank == 1)
                ...
                if (line != null && line.getClass().isArray()) {
    ...
    public static <T> void writeArrayToJTable(JTable table, T[] array) {
    ...
    public static <T> T[][] readMatrixFromJTable(
        ...
        for (int r = 0; r < rowCount; r++) {
            for (int c = 0; c < colCount; c++) {
                ...
                if (obj == null || obj instanceof String str && (str.trim().length() == 0)) {
                    if (errorIfEmptyCell) {
    ...
    public static <T> T[] readArrayFromJTable(JTable table, Class<T> clazz, Function<String, ? extends T> converter) throws JTableUtilsException {
    ...
    private static class ReadingException extends RuntimeException {

        public ReadingException(JTableUtilsException ex) {
            super(ex);
        }
    }
}